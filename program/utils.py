...
class fusion_depth:
    ...
    def hipass(self):
        ...
        fusion_image = self.fg_image * mask_fg + self.bg_image * mask_bg
        return fusion_image
...
class simulate_abnormal_vision:
    ...
    def red_green_blind(self):
        ...
        Lab_image = cv2.cvtColor(Lab_image, cv2.COLOR_Lab2BGR) * 255
    ...
    def glaucoma(self, sigma):
        ...
        center_point = (gaussian_filter.shape[0] / 2, gaussian_filter.shape[1]/2)
        ...
        gaussian_filter = np.concatenate([gaussian_filter, gaussian_filter, gaussian_filter], axis=2)
    ...
    def plot_statistics(self):
        ...
        for key, pair in key_pair.items():
            ...
            for idx in range(len(self.image_path)):
                ...
                x_start = np.ceil((feature_statistic[pair[1]][idx] - min_x) * (Max / (max_x - min_x))).astype(np.int16)
                ...
                leave_position_data = background[background.shape[0] - 1 - x_start - image.shape[0]:background.shape[0] - 1 - x_start, y_start:y_start + image.shape[1] , :]
                leave_position_data = leave_position_data * inverse_mask + image * mask


                background[background.shape[0] - 1 - x_start - image.shape[0]:background.shape[0] - 1 - x_start, y_start:y_start + image.shape[1] , :] = leave_position_data
